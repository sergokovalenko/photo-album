...
class CommentsContainer extends Component {
    ...
    componentDidMount() {
        ...
        fetch(`${window.host}/api/comment/getCommentsByAlbumId/${albumId}`, {
            ...
            .then((res) => {
                this.setState({ comments: res.reverse() });
                ...
                Promise.all(fetches).then((users) => {
                    ...
                    this.setState({ fullComments: mapped });
                });
            })
            .catch(() => {
    ...
    sendComment = () => {
        const { fullComments, commentValue } = this.state;
        const { curUserId, albumId } = this.props;
        ...
        fetch(`${window.host}/api/comment/createComment`, {
            ...
            .then((res) => {
                fetch(`${window.host}/api/user/${res.user}`, {
                    ...
                    .then(user => {
                        const a = [...fullComments];
                        a.push({ ...user, ...res });
                        this.setState({ fullComments: a });
                    })
            })
            .catch(() => {
                alert('comment wasn\'t sent')
    ...
    render() {
        const { fullComments, comments, commentValue } = this.state;
        ...
        return (
            <div className="comments mt-3 mb-5">
                <div className="form-row">
                    ...
                    <div className="col">
                        <button onClick={() => this.sendComment()} className="btn btn-primary">Comment</button>
                    </div>
                </div>
                {
                    fullComments.length ?
